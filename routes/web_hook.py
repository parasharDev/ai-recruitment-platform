from fastapi import APIRouter, Request
import json
import threading

router = APIRouter()
VERIFY_TOKEN = "my_verify_token"


@router.get("/whatsapp/webhook")
async def verify(request: Request):
    mode = request.query_params.get("hub.mode")
    token = request.query_params.get("hub.verify_token")
    challenge = request.query_params.get("hub.challenge")
    if mode == "subscribe" and token == VERIFY_TOKEN:
        return int(challenge)
    return {"status": "verification failed"}


@router.post("/whatsapp/webhook")
async def webhook(request: Request):
    data = await request.json()
    print("\nIncoming WhatsApp:", json.dumps(data, indent=2))

    try:
        msg = data["entry"][0]["changes"][0]["value"].get("messages", [])[0]
        from_number = msg.get("from")

        if msg.get("type") != "interactive":
            return {"status": "ignored"}

        button_id = msg["interactive"]["button_reply"]["id"]
        selected_index = int(button_id.split("_")[1]) - 1

        # fetching cache
        from routes.whatsapp_send import SLOT_CACHE
        cache = SLOT_CACHE.get(from_number)
        if not cache:
            return {"status": "cache_not_found"}

        selected_slot = cache["slots"][selected_index]
        candidate_email = cache["candidate_email"]
        job_title = cache["job_title"]

        # running schedule email+meet link in BACKGROUND
        def schedule_in_background():
            from utils.schedule_interview_email import schedule_interview_email
            meet_link, html_link = schedule_interview_email(
                selected_slot, candidate_email, job_title, from_number
            )
            if not meet_link:
                return

            cache.update({
                "selected_slot": selected_slot,
                "meet_link": meet_link,
                "html_link": html_link,
                "status": "scheduled",
            })

            from utils.whatsapp_client import send_text
            send_text(from_number, f"Interview confirmed!\nMeet Link: {meet_link}")

            from utils.feedback_email import send_feedback_email
            send_feedback_email(
                candidate_name=cache["candidate_name"],
                job_title=job_title,
                selected_slot=selected_slot,
            )

        threading.Thread(target=schedule_in_background).start()

        return {"status": "processing"}  # instant response

    except Exception as e:
        print("ERROR:", e)
        return {"status": "error", "message": str(e)}



